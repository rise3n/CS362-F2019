...
int refactored_ambassador(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos) {
	...
	result = ambassadorEffect(choice1, choice2, handPos,  state, currentPlayer);
	...
	assert(post == (pre + 1));// check if supply has more cards, should be 1 more(2 copies and just 1 other player). If not pass then the bug 2 is exposed.
	endTurn(state);
}
